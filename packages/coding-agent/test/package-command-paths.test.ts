...
describe("package commands", () => {
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("motto install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "motto --help" or "motto install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: motto install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`motto is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated motto from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated motto from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated motto");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `motto update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
