...
- **I6-4** 任何 Motto 渲染/命令不得写入装饰字符或显示前缀到剪贴板。适用范围就地
  ...
  前缀；**拖选路径**按 I6-3 视觉行 join 复制，S1 首行短横衬线 `─────────` 为显示投影、会随拖选
...
无居中。装饰堆叠即噪声。唯一例外：transcript user 消息左上方短横衬线 `─────────`(tui-1-s1-r1/r2/r3) + 正文续行悬挂缩进
...
- **I8-3** 渲染输出中无 `┌─┐│└┘` 等表格边框以外的装饰字符（表格边框为原生数据结构
  呈现，豁免；transcript 首行短横衬线 `─────────` 属 I6-4 就地界定的显示投影，一并豁免）。
...
- I6-4 修订（2026-08-11，S1 首行界栏；2026-08-13 tui-1-s1-r1 改短横衬线；tui-1-s1-r2 加长 3→5 × U+2500；tui-1-s1-r3 加长 5→9 × U+2500）：衬线 `─────────`(tui-1-s1-r1/r2/r3)独立成行，正文全部续行两空格悬挂缩进，
