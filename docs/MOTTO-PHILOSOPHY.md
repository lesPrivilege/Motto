...
> **立场修订（2026-08-12，用户校准）**：施工不是为 fork Pi 做微调，而是**实践本理念**——
> 在极简 harness 之上以发行/版式/取舍立自己的层。长期姿态是**持续吸纳上游与生态**
> （上游 remote + patch 逐条登记重放 + EXTENSIONS.lock 生态钉版），凡不能保持吸纳通道的
> 结构，**勿增实体**——删结构优于加兼容面，不保留会阻碍吸收的中间件。
>
> **立场修订（2026-08-13，用户定调）**：core 权限开放，motto 进化为**薄而自足的 harness**；
> 优先不随模型 agentic 能力提升而折旧的设计，当下正是弃置约束类 harness 的节点；
> coding is cheaper——取舍先于实现。详见 `docs/decisions/2026-08-13-motto-thin-harness.md`。
...
5. **选极简视觉（无框、无竖线、无装饰、朱墨三用、灰阶、左锚），而非任何装饰堆叠。**
   ...
   校记（失败），Motto 新增投影层与主题无第四种红、无绿、无 ✓/×、无 success/error 语义色
   （上游 legacy 选中/成功标记为未改动原码，Motto 主题下渲染为 mid 中灰，不构成语义色）。界栏、悬挂、著录、
...
9. **选不折旧的长期设计，而非随模型 agentic 能力提升而折旧的约束层（2026-08-13）。**
   版式、排版、信息呈现、目验标准、投影层——能力越强越值钱；约束类 harness（因 agentic 能力
   不够而添加的 prompt 约束、防御性流程、行为矫正层）随能力提升折旧——当下正是弃置节点，
   逐项清退、不新增。core 权限开放（2026-08-13 裁定）：不再限于薄接缝，但纪律不变——解耦最小、
   登记 PATCHES、removalCondition、可独立回退；agent loop / provider / session schema /
   内置工具语义默认零修改，改动须独立裁定。coding is cheaper：取舍先于实现，便宜不等于该做，
   准入仍守 ROADMAP 三类合法依据。TUI 版式学吸收目录版本之学的原理层（源流考辨、善本目验），
   三不：不借词语、不借形体、不牵附——唯一准绳是人类目验 clean and cool。
   理由：「慎厥身修思永」——修身为本、图谋长远；为约束而生的代码不该长命，为阅读而生的版式
   应当长寿。〔R, M, 2026-08-13-thin-harness〕
